...
class Questionario(db.Model):
    ...
    questions = db.relationship('Domanda', backref=db.backref('domande', collection_class=list))
...
class DomandaForm(NoCsrfForm):
    testo = StringField('Title', validators=[validators.DataRequired('please, enter the question')])


class QuestionarioForm(FlaskForm):
    titolo = StringField('Titolo')
    questions = FieldList(FormField(DomandaForm, default=lambda: Domanda()), min_entries=1)
...
def index():
    ...
    return render_template('index.html', form=form, _template=domande, questionario=quiz)
